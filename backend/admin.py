...
class UserAdmin(ModelAdmin, ImportExportModelAdmin):
    ...
    def formatted_email(self, obj):
        return format_html(
            '<span class="font-semibold text-primary-600">{}</span>', obj.email
        )
...
class ShopAdmin(ModelAdmin):
    ...
    def formatted_name(self, obj):
        return format_html(
            '<span class="font-semibold text-primary-600">{}</span>', obj.name
        )
    ...
    def products_count(self, obj):
        ...
        return format_html('<span class="font-medium text-gray-700">{}</span>', count)
...
class ProductAdmin(ModelAdmin):
    ...
    def parameters_count(self, obj):
        count = obj.info.aggregate(param_count=Count("parameters"))["param_count"] or 0
        return format_html('<span class="font-medium text-gray-700">{}</span>', count)
...
class ProductInfoAdmin(ModelAdmin):
    ...
    def formatted_article(self, obj):
        return format_html(
            '<span class="font-semibold text-primary-600">{}</span>', obj.article
        )
...
class ProductParameterAdmin(ModelAdmin):
    ...
    def product_info_link(self, obj):
        if obj.product_info:
            url = reverse(
                "admin:backend_productinfo_change", args=[obj.product_info.id]
            )
            return format_html(
                '<a href="{}" class="text-primary-600 hover:text-primary-800">{}</a>',
                url,
                f"{obj.product_info.product.name} @ {obj.product_info.shop.name}",
            )
        return "-"
...
class OrderAdmin(ModelAdmin, ImportExportModelAdmin):
    ...
    def formatted_id(self, obj):
        return format_html(
            '<span class="font-semibold text-primary-600">#{}</span>', obj.id
        )
    ...
    def total_amount(self, obj):
        ...
        return format_html(
            '<span class="font-semibold text-green-600">{} ₽</span>', f"{total:.2f}"
        )
    ...
    def contact_link(self, obj):
        if obj.contact:
            ...
            return format_html(
                '<a href="{}" class="text-primary-600 hover:text-primary-800">{}</a>',
                url,
                f"{obj.contact.last_name} {obj.contact.first_name}",
            )
        return "-"
...
class CartAdmin(ModelAdmin):
    ...
    list_filter = (("updated_at", RangeDateFilter),)
...
class CartItemAdmin(ModelAdmin):
    ...
    list_filter = (("quantity", ChoicesDropdownFilter),)
...
class ContactAdmin(ModelAdmin):
    ...
    list_filter = (("city", ChoicesDropdownFilter),)
...
class EmailConfirmTokenAdmin(ModelAdmin):
    ...
    def token_preview(self, obj):
        ...
        return format_html(
            '<span class="font-mono text-sm text-gray-600">{}</span>', preview
        )
...
admin.site.site_title = getattr(
    settings, "ADMIN_SITE_TITLE", "Netology Shop Admin Portal"
)
admin.site.index_title = getattr(
    settings, "ADMIN_INDEX_TITLE", "Welcome to Netology Shop Admin Portal"
)
